    ...
    try {
      ...
      for (const rawLine of lineItems) {
        ...
        if (costPerItem?.amount) {
          price = { amount: String(costPerItem.amount), currencyCode: costPerItem.currency ?? costPerItem.currencyCode ?? 'USD' };
        }
    ...
    search_shop_catalog: tool({
      ...
      execute: async ({ query, context }) => {
        try {
          ...
          const rawContent = await callShopifyMCPRaw(shopDomain, 'search_shop_catalog', { query, context: context || 'general product search' });
