...
function safeName(tenantId: string): string {
  return `tenant_${tenantId.replace('store_', '').replace(/[^a-z0-9_]/g, '_')}`;
}
...
const knowledgeRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/knowledge', async (request, reply) => {
    const { shopDomain, tenantId } = request;
    const s = safeName(tenantId);
  ...
  app.get('/knowledge', async (request, reply) => {
    const s = safeName(request.tenantId);
