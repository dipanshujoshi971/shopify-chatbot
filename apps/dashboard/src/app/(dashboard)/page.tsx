...
async function getOverviewData(merchantId: string) {
  ...
  try {
    const [convToday, convWeek, msgWeek, tokenWeek, active, recentConvs, convPrevWeek, msgPrevWeek] =
      await Promise.all([
        ...
        pgPool.unsafe(`SELECT id, session_id, status, total_turns, total_tokens_used, created_at FROM "tenant_${sn}"."conversations" ORDER BY created_at DESC LIMIT 6`),
    ...
    return { convToday: 0, convWeek: 0, msgWeek: 0, tokenWeek: 0, active: 0, recentConvs: [], convPrevWeek: 0, msgPrevWeek: 0 };
  }
}
          ...
          >
            ...
            {s.trend !== null && (
              <div className="flex items-center gap-1 mt-2 relative z-10">
                ...
                <span className={`text-xs font-semibold ${s.trend >= 0 ? 'text-emerald-500' : 'text-red-400'}`}>
                  {Math.abs(s.trend)}%
                </span>
        ...
        <div className="lg:col-span-3 glass-card overflow-hidden">
          ...
          {data.recentConvs.length === 0 ? (
            <div className="px-6 py-16 text-center">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">Share your widget to start chatting!</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">
                        Session {conv.session_id.slice(0, 10)}...
                      </p>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="glass-card p-6">
            ...
            <div className="space-y-2.5">
              {[
                { href: '/integration/ai-playground', icon: Bot, title: 'AI Playground', desc: 'Test and tune your bot', color: 'text-emerald-500 bg-emerald-500/10' },
                { href: '/integration/installation', icon: ShoppingCart, title: 'Install Widget', desc: 'Add chatbot to your store', color: 'text-blue-500 bg-blue-500/10' },
                { href: '/analytics/conversational', icon: TrendingUp, title: 'View Analytics', desc: 'Conversation insights', color: 'text-violet-500 bg-violet-500/10' },
                { href: '/tickets/merchant-support', icon: DollarSign, title: 'Support Tickets', desc: 'Manage escalations', color: 'text-amber-500 bg-amber-500/10' },
              ].map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
