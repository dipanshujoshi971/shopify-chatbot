 ...
 * Matches the path referenced in package.json: "node scripts/test-server.js"
 *
 * What it provides:
 *   GET  /             → mock Shopify storefront with widget embedded
 *   POST /widget/chat  → mock streaming chat API (no real LLM)
 *   GET  /widget.iife.js → serves the built bundle from dist/
 *
 * Usage:
 *   1. Build widget:   pnpm --filter @chatbot/widget build
 ...
 *   "hello!"            → Streaming plain text
 *
 * Env overrides:
 *   TEST_PORT=4000
 *   MOCK_PRODUCTS=1   → force product carousel on any message
 *   MOCK_ORDER=1      → force order card on any message
 */
...
// dist/ is one level up from scripts/
...
async function* mockStream(message) {
  ...
  if (lower.includes('product') || lower.includes('sweater') || lower.includes('show') || process.env.MOCK_PRODUCTS === '1') {
    yield encode('0', 'Here are some products I found that might interest you:');
    await delay(400);
    yield encode('2', [{ type: 'tool_result', toolName: 'search_shop_catalog', toolCallId: 'tc_mock_1', result: MOCK_PRODUCTS }]);
  ...
  if (lower.includes('order') || lower.includes('track') || lower.includes('delivery') || process.env.MOCK_ORDER === '1') {
    yield encode('0', "I found your order. Here's the current status:");
    await delay(300);
    yield encode('2', [{ type: 'tool_result', toolName: 'get_order_status', toolCallId: 'tc_mock_2', result: MOCK_ORDER }]);
  ...
  // Generic streaming text
  const words = ["Thanks", " for", " reaching", " out!", " I'm", " the", " AI", " assistant", " for", " this", " store.", " I", " can", " help", " you", " find", " products,", " check", " order", " status,", " and", " answer", " questions", " about", " our", " policies.", " What", " would", " you", " like", " to", " know?"];
...
const server = http.createServer(async (req, res) => {
  ...
  // CORS headers (dev only — production uses widgetAuth domain-lock)
  ...
  if (url.pathname === '/widget.iife.js') {
    ...
    if (!fs.existsSync(filePath)) {
      ...
      res.end('Widget not built. Run: pnpm --filter @chatbot/widget build');
      return;
    }
  ...
  // Mock chat endpoint — validates X-Widget-Key (matches widgetAuth.ts behaviour)
  if (url.pathname === '/widget/chat' && req.method === 'POST') {
    ...
    // Validate required fields (mirrors chat.ts schema)
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .store-header { background: #fff; border-bottom: 1px solid #e2e8f0; padding: 16px 24px; display: flex; align-items: center; justify-content: space-between; }
    .store-logo { font-size: 20px; font-weight: 700; color: #059669; }
    .store-nav  { display: flex; gap: 24px; font-size: 14px; color: #64748b; }
    .store-hero { padding: 80px 24px; text-align: center; background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 50%, #6ee7b7 100%); }
    ...
    .hero-btn { display: inline-block; padding: 14px 32px; background: #059669; color: #fff; border-radius: 8px; font-size: 16px; font-weight: 600; text-decoration: none; }
    ...
    .product-card { background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; }
    ...
    .test-controls { position: fixed; top: 70px; left: 16px; background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 12px; font-size: 12px; color: #64748b; box-shadow: 0 2px 8px rgba(0,0,0,.06); z-index: 9999; max-width: 200px; }
...
<body>
  ...
  <div class="test-controls">
    ...
    <p class="test-hint">Try: <code>hello!</code> (plain text)</p>
    <p class="test-hint" style="margin-top:8px;color:#059669">Key: <code>pk_test_mock</code></p>
  </div>
...
server.listen(PORT, () => {
  console.log(`
╔════════════════════════════════════════════════════╗
║   Shopbot Widget Test Server (Fixed)               ║
╠════════════════════════════════════════════════════╣
║   Storefront → http://localhost:${PORT}               ║
║   API        → POST /widget/chat                   ║
║   Bundle     → GET  /widget.iife.js                ║
╠════════════════════════════════════════════════════╣
║   Fixed: validates X-Widget-Key (not X-Api-Key)    ║
║   Fixed: validates sessionId in body               ║
╚════════════════════════════════════════════════════╝
`);
});