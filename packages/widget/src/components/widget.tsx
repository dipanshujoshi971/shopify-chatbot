...
function MessageBubble({ msg, onSendMessage }: { msg: ChatMessage; onSendMessage?: (text: string) => void }) {
  ...
  // Don't show product carousel alongside cart/order — products were just
...
export function Widget({ config }: WidgetProps) {
  ...
  const sendMessage = useCallback(async (overrideText?: string) => {
    ...
    try {
      ...
      // Track cartId for future cart operations
  ...
  }, [input, loading, apiBaseUrl, apiKey, shopDomain, sessionId, conversationId, cartId, messages.length, pageCtx]);
